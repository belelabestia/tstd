...
- **side quests**: `?none`, `?some`, `?==`, `?!=`, `?>`, `?<`, `?>=`, `?<=`, `?&(...)`, `?|(...)`, `?:tag`, `?(cond)`, bare `?` and `?!`, plus the `? {}` block for exhaustive answers. each one tests one thing at the position where it stands. a side quest that finds its match exits or arrow captures; a side quest that misses continues unless the chain ends. `?true`, `?false` and `?literal` retired into `?==`; juxtaposed chains join in groups now.
