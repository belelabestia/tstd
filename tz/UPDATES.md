...
three notes the author raised at the close of session 05 are now scheduled. the comment channel is a living protocol, not a phase: comments carrying forward work are part of the no-debt approach, and session 07 wrote the protocol rather than deleting the notes. `form` gets an audit (q8, session 16) because its emit is inconsistent, some code written and some emitted along an arbitrary line. a `test` keyword like zig's, in the file it tests and reaching module internals, is explored in session 17 (q9). the `if` overlap is now session 18, ruled in the dissertation: `switch` is already banned, `if` is not, and every `if` is replaceable by a side quest. a new session 10, the core subset, was added ahead of the battle phase to name the shared floor both `tstd` and tz sit on and to land the editor smell warning that deprecation by default implies.
