...
three notes the author raised at the close of session 05 are now scheduled. the comment channel is a living protocol, not a phase: comments carrying forward work are part of the no-debt approach, and session 07 wrote the protocol rather than deleting the notes. `form` gets an audit (q8, session 16) because its emit is inconsistent, some code written and some emitted along an arbitrary line. a `test` keyword like zig's, in the file it tests and reaching module internals, is explored in session 17 (q9). the `if` overlap is now session 18, ruled in the dissertation: `switch` is already banned, `if` is not, and every `if` is replaceable by a side quest. a new session 10, the core subset, was added ahead of the battle phase to name the shared floor both `tstd` and tz sit on and to land the editor smell warning that deprecation by default implies. an addendum after the close adds session 19, the value declaration: the author objects to `form` and `protocol` sitting where a `function` or `class` keyword sits, and wants `export const signup = form { ... }` instead, with no `=>` because `{` is an object literal rather than a scope. it is a discussion, plans no execution, and owns the schedule of whatever it authorizes. session 10 followed and landed: the shared floor is named typecore, its profile is one table both engines read, and the smell warning ships in `tzd`. session 11 followed and landed: robust and reliable now have a written standard, eight checkable criteria and three measurable conditions for the split; the next session is 12, choose the target.

## 2026-09-23: define robust

session 11 writes the standard. robust and reliable had no testable meaning, and "done" meant `npm test` is green, which phase 0 showed is not the same as the language working. the two words split: **robust** is the build telling the truth (it runs what it emits, the checker is right or silent, a refusal names its replacement, the surface is coherent); **reliable** is the same source giving the same named outcome, with nothing dropped, swallowed, or decided silently.

eight criteria, each checkable by a command or a person in one sitting. six are met: the build runs the code (01), a diagnostic lands on the `.tz` line (04), the checker is silent or right (04, 10), a refusal names its replacement, the coherence walk passes (6/6, with the `if` overlap as the named open for session 18), and the editor loop is honest (the interim surface is documented as shelling out). two are owed to the battle test: a real program runs in the build (session 13), and a newcomer follows the tutorial to a running program without a line that lies (session 13, sharpened in 14).

the first build is done when the program is a whole problem taking outside input, runs under `npm test`, uses the leaky group (`scope`, `call`, `form`, `protocol`, `make`) by use, and its friction is written down as ledger entries.

q5 is answered: stable enough to split means three conditions, all measurable. the dependency is a version, not a path (no `paths` into `../src`, no `file:..`); a fresh clone of `tz/` alone, against a tagged `tstd` from the registry, passes `npm test` with no change to either side; and the ban table, the role list and the emitted shapes hold unchanged across one full `tstd` release while the battle program is built.

the standard lives in `plan/context/11-define-robust.md`; no new doc, so the coherence walk is untouched. the author ruled the four drafts in review: two words, not one; the three-condition split test, not two; the newcomer criterion in now, not deferred; the context file as its home, not a `tz/ROBUST.md`.

what changed: `plan/context/11-define-robust.md` (the standard), `plan/roadmap.md` (session 11 done, q5 resolved in the ledger), `UPDATES.md`.

what verified: no code changed. `npm test` at the root is exit 0 at 43; in `tz/` exit 0 at 69 src plus 54 decks and examples; `tzc examples constructs` and `tzd examples constructs` both exit 0.

## 2026-09-23: the core subset

session 10 names the floor `tstd` and tz sit on. the author first named it corescript, rejected it on collision, tried typego, goscript and subtype (each occupied in the typescript or type-theory space), and ruled **typecore**: a conformance profile, not a file type, so files stay `.ts` and `.tz` and no extension is spent. the three things stay distinct: typecore is the base dialect, `tstd` is the standard library, and tz is the language that adds the constructs and enforces the profile. `tstd` does not mean "typescript with the ruleset baked in"; the seed stays the seed.

the profile is one table, `src/typecore.ts`, holding the refused words and the warned library spellings. the ban pass (`src/ban.ts`) and the editor warning (`src/smell.ts`) both read it, so the two engines cannot drift. the eslint ruleset that holds plain typescript to typecore reads the same table; it is a consumer rather than a second definition, and it is scheduled, because building it here would add eslint to the repo against `AGENTS.md`. the tz pass is defensive: the profile decides, tz insists.

the gradient got its words. a spelling the language owns is banned, a spelling the library owns is warned; "deprecated" is retired. the smell warning is real work rather than intent: the fourth `tzd` check reads tokens, not types, and names each library spelling (`protocol.init`, `branch`, `result.ok`/`result.err`, `scope.sync`/`scope.async`, `call.sync`/`call.async`, `make`) with the tz form it should be. it is `TZL0005`.

the framing landed across `README.md`, `tz/README.md`, `TUTORIAL.md` and `DESIGN.md`; a new `## typecore` section in `DESIGN.md` states the profile and the enforcement split, the lsp section now describes four checks, and the staging note reads four. a `.tz` file reads as tz, so the lowering comparisons left the showcase by follow-up ruling: the tautological `assert.deepEqual(x, branch('x'))` examples are for a reader of the lowered output, not someone reading tz, so `result.ok`/`result.err` became `:ok`/`:err`, `branch('x', ...)` became `:x(...)`, and the comparisons became `.branch`/`.value` reads. the `branch` role's coherence anchor moved from `branch(` to `:idle`, because the old anchor required the deck to contain the very library spelling the warning flags. `tzd examples constructs` is now silent.

what changed: `src/typecore.ts`, `src/smell.ts` and `src/smell.spec.ts` (new), `src/ban.ts` (reads the profile), `src/lsp.ts` and `src/lsp.spec.ts` (the warning joins the session), `src/coherence.spec.ts` (typecore whitelisted, the profile import), `src/emit.ts` (the `branch` role anchor), `constructs/block.spec.tz`, `constructs/branch.spec.tz`, `constructs/tag.spec.tz`, `examples/signup.spec.tz` (the lowering comparisons dropped), `README.md`, `tz/README.md`, `TUTORIAL.md`, `DESIGN.md`, `src/emit.spec.ts`, `plan/roadmap.md`, `plan/context/10-the-core-subset.md`.

what verified: `npm test` at the root is exit 0 at 43; in `tz/` exit 0 at 69 src plus 54 decks and examples; `tzc examples constructs` exit 0; the `tzx` spec is 54 pass; `tzd examples constructs` exit 0 with no warnings; the coherence spec is 6/6.

## 2026-09-23: the educational rewrite

session 09 executes the organization session 08 settled. the tutorial is now language-first: the genesis (`where we come from` and `how things are in typescript`) left it for `DESIGN.md`, the vocabulary moved ahead of the refusals, the constructs run closed (`return`, `ok`, `err`, `async` exits, arrow capture, side quests, `try`) then leaky (`scope`, `protocol`, `form`, `call`, `make`) with each leaky one marking its `tstd` bond, and the construct matrix moved to the end. the lowered-core transcriptions left the tutorial (08: the lowered core belongs in the decks and `DESIGN.md`), so the `which emits` blocks, the `form` derived-`.ts` block and the `protocol` scaffold table are gone. the tutorial is 578 lines, inside 08's 550 to 580 target.

the formula changed. the author retired "typescript with most of typescript taken away" for "a superset of a subset of typescript, with its own standard library, out of the box": `tstd` is the seed that carries every principle and works in plain typescript, and tz is the language built to optimize that usage past what typescript alone reaches. the line now lives in `TUTORIAL.md`, `DESIGN.md`, `tz/README.md`, the root `README.md` and the `emit.spec.ts` essay. the genesis was not left as a relocated chapter: it folds into the design notes' `## hard constraints`, in the new voice. two smaller corrections landed with it: `return`, `err` and `ok` carry a value when one is named while `break` and `continue` only leave, and "it isn't in the language" became "it is plain typescript, left as-is". the door (`tz/README.md`) gained the dense mixed example session 08 assigned it (`form`, `scope`/`hold`, `try`, `await call`, a side quest, `ok`).

the library side got its own word. the language spellings are banned; the library spellings (`protocol.init`, `branch`, `result.ok`, `scope.sync`, `call.sync`, `make`) are warned by the editor, and that `tzd` smell warning is no longer intent but scheduled work.

a new session 10, the core subset, was pulled ahead of the battle phase to name the shared floor both `tstd` and tz sit on. the author named it corescript, with `cts` only a candidate extension; no extension is spent, because `.cs` collides with c# and `.cts` with typescript's own CommonJS flavour, so files stay `.ts`. the session also settles whether `tstd` reads as "typescript with the ruleset baked in", and lands the smell warning. sessions 10 to 17 shifted to 11 to 18, and every cross-reference was remapped.

one defect swept on the way: the session prompts read `../AGENTS.md` and `../STYLE-KB.md`, which resolve to nothing (the files are at the root, two levels up from `tz/plan`). the prompts now read `../../AGENTS.md` and `../../STYLE-KB.md`.

what changed: `TUTORIAL.md` (the rewrite, 663 to 578 lines), `DESIGN.md` (the folded why, the formula), `tz/README.md` (the formula, the dense example), `README.md` (the tz pointer), `src/emit.spec.ts` (the essay), `src/coherence.spec.ts` (`using` whitelisted), `plan/roadmap.md` (session 10, the 10 to 17 shift, the 09 status), `plan/sessions/` (10 new, 10 to 16 renamed to 11 to 17, the prompt paths swept), `plan/context/09-the-educational-rewrite.md`.

what verified: `npm test` at the root is exit 0 at 43; in `tz/` exit 0 at 65 src plus 54 decks and examples; `tzc examples constructs` and `tzd examples constructs` both exit 0; the coherence spec is 6/6. the tutorial is 578 lines, inside the 550 to 580 target.
