...
- a getting-started path: install, first program, run it, the mental model. short, and true to what session 11 actually required.
...
- the tutorial's internal organization, which session 07 settled and session 08 wrote.
- agent onboarding (session 13).

## inputs

- `../README.md`.
- the session 11 friction journal, specifically the newcomer's stumbles.
...
> read `../AGENTS.md` and `../STYLE-KB.md` first, then the roadmap, the session 11 friction journal, and this file. copy `session-context-template.md` to `context/12-onboard-people.md`. walk the first-run path from an empty directory and record every stumble, then write a getting-started document true to that walk. rule on when and how tz becomes installable, and trim the README to its entry-point role. do not commit until the author reviews.

## context

copy `../session-context-template.md` to `../context/12-onboard-people.md` before starting.
