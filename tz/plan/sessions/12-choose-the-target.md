...
- the acceptance criteria from session 11 mapped onto the program.
...
- the standard from session 11.
- `../README.md:45`, what the language says it already does.
- the author's goal: web development and scripting.

## steps

1. put the candidates beside the session 11 criteria and show which surface each misses.
...
- every session 11 criterion is either exercised or explicitly not covered.

## starting prompt

> read `../../AGENTS.md` and `../../STYLE-KB.md` first, then the roadmap, the session 11 standard, and this file. copy `session-context-template.md` to `context/12-choose-the-target.md`. help the author choose the first real program to build in tz, weigh the candidates (web service, cli, scripting task) against the robustness criteria and the surfaces they leave untested, and write a one-page brief with the constructs under test and the definition of a first version. do not commit until the author reviews.

## context

copy `../session-context-template.md` to `../context/12-choose-the-target.md` before starting.
