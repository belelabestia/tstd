...
- onboarding material. that is sessions 12 and 13, extracted after the build.

## inputs

- the target brief from session 10.
- the robustness standard from session 09.
- the tutorial and `../scratch/`, for the patterns.

## steps

1. scaffold the target and get one path running end to end.
2. build against the brief, keeping the friction journal current.
3. run the session 09 criteria against the result.
...
- the session 09 criteria are measured and reported, pass or fail.

## starting prompt

> read `../AGENTS.md` and `../STYLE-KB.md` first, then the roadmap, the session 09 standard, the session 10 brief, and this file. copy `session-context-template.md` to `context/11-the-first-build.md`. build the first version of the target in tz. keep a friction journal with exact code for every place the language fought back, and a ranked, evidenced list of candidate changes. do not fix the language mid-build; file and route around blockers. measure the session 09 criteria and report them. do not commit until the author reviews.

## context

copy `../session-context-template.md` to `../context/11-the-first-build.md` before starting.
