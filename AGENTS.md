...
cross-tool onboarding for ai coding agents. this is the working-rules file; the principles live in `README.md`.
...
- `STYLE-KB.md`: worked entries, each grounded in a real line here, in ❌ instead of / ✅ do form. **read it before writing anything**, including prose. it also carries the open work at the top.
...
- `STYLE-KB.md`: the worked entries. the examples that ground the rules. read before writing anything.
- `AGENTS.md`: this file. the working rules for an agent that touches this repo. the code rules, the commit rules, the things to refuse.
...
- read `README.md` and `STYLE-KB.md` first. they are short.
...
these compress `STYLE-KB.md`. go there for the examples.
