...
test('return a result', () => {
  ...
  const dangerousAction = (n: number) => {
    if (n < 5) return result.err('not enough');
    if (n > 10) return result.ok('incredible');
    return result.ok();
  };
  ...
  assert.deepEqual(res, result.ok('incredible'));
});
...
test('execute safely with the call api', () => {
  ...
  assert.deepEqual(res.branch, 'err');
});
